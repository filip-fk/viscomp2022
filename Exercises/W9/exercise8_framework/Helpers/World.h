...
public:
	...
	void setViewMatrix(const Matrix &val) { m_viewMatrix = val; }

	const Matrix &getProjectionMatrix() const { return m_projectionMatrix; }
	Matrix &getProjectionMatrix() { return m_projectionMatrix; }
	void setProjectionMatrix(const Matrix &val) { m_projectionMatrix = val; }
	...
	const Vector &getLightPosition(unsigned int light) const {
		assert(light < WORLD_NUMLIGHTS);
		return m_lightPositions[light]; 
	}

	Vector &getLightPosition(unsigned int light) {
		assert(light < WORLD_NUMLIGHTS);
		return m_lightPositions[light]; 
	}
